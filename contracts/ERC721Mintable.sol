...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(_owner == msg.sender, "The account is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        require(
            !newOwner.isContract(),
            "The new Owner should not be a contract"
        );
        _owner = newOwner;
        emit OwnerTransfered(_owner);
    }
...
contract Pausable is Ownable {
    ...
    modifier requireDiffPause(bool mode) {
        if (mode == true) {
            require(_paused != mode, "The contract is already paused");
            emit Paused(msg.sender);
        } else {
            require(_paused != mode, "The contract is already unpaused");
    ...
    function setPause(bool mode) public onlyOwner requireDiffPause(mode) {
        _paused = mode;
    }
}
...
contract ERC165 {
    ...
    function _registerInterface(bytes4 interfaceId) internal {
        require(interfaceId != 0xffffffff, "InterfaceId wrong");
...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public {
        ...
        require(
            to != ownerOf(tokenId),
            "The address for approval is the owner"
        );
        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(
            msg.sender == owner || isApprovedForAll(owner, msg.sender),
            "The sender is not authorized to approve"
        );
    ...
    function setApprovalForAll(address to, bool approved) public {
        require(to != msg.sender, "Operator cannot approve self");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(!to.isContract(), "Given address is invalid");
        // TODO mint tokenId to given address & increase token count of owner
        _ownedTokensCount[to].increment();
        _tokenOwner[tokenId] = to;
        // TODO emit Transfer event
        emit Transfer(ownerOf(0), to, tokenId);
    }
    ...
    function _transferFrom(
        ...
        require(
            ...
            "From address is not the owner of token"
        );
        // TODO: require token is being transfered to valid address
        require(!to.isContract(), "To address is not valid address");
        // TODO: clear approval
...
contract ERC721Enumerable is ERC165, ERC721 {
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index)
        ...
        returns (uint256)
    {
        require(index < balanceOf(owner), "Index not less than balance");
    ...
    function tokenByIndex(uint256 index) public view returns (uint256) {
        require(index < totalSupply(), "Index not less than total supply");
